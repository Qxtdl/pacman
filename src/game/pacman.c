...
void pacman_tick(void) { 
    if (IsKeyDown(KEY_W)) game.pacman.direction = DIRECTION_UP;
    if (IsKeyDown(KEY_A)) game.pacman.direction = DIRECTION_LEFT;
    if (IsKeyDown(KEY_S)) game.pacman.direction = DIRECTION_DOWN;
    if (IsKeyDown(KEY_D)) game.pacman.direction = DIRECTION_RIGHT;
    ...
    if (timer_triggered(SLOT_PACMAN_MOVE, SLOT_PACMAN_MOVE_VALUE)) {
        ...
        switch (direction) {
            ...
            case DIRECTION_UP: new_pos_y = game.pacman.pos_y - 1; break;
            case DIRECTION_LEFT: new_pos_x = game.pacman.pos_x - 1; break;
            case DIRECTION_DOWN: new_pos_y = game.pacman.pos_y + 1; break;
            case DIRECTION_RIGHT: new_pos_x = game.pacman.pos_x + 1; break;
        }
